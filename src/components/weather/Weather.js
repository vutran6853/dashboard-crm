...
function Weather(props) {
  ...
  useEffect(() => {
    axios.get(`https://api.weatherbit.io/v2.0/current?&postal_code=${ zipcode.defaultZipcode }&key=${ process.env.REACT_APP_WEATHER_KEY }`)
  ...
  useEffect(() => {
    axios.get(`https://api.weatherbit.io/v2.0/forecast/daily?&postal_code=${ zipcode.defaultZipcode }&key=${ process.env.REACT_APP_WEATHER_KEY }`)
    .then((response) => {
      let result = []
  ...
  return (
    <div className="weather_container">
      <div>
        <input type="text" placeholder="Enter zipcode..." value={ zipcode.newZipcode } onChange={ handleSetNewZipcode } maxLength={ 6 } />
