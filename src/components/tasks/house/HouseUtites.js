...
function HouseUtites(props) {
  ...
  const displayGrapicType = props.grapic.grapicType.map((value) => <option key={value}>{value}</option>)

  const displayYear = range_date.map((value) => <option key={value}>{value}</option>)
  ...
  return (
    <div className="house_utites__overall_form_container">
      <div>
        <select onChange={handleSetSelectTypeName}>{displayTypeName}</select>

        <select onChange={handleSetSelectGrapicType}>{displayGrapicType}</select>
      ...
      <div className="house_utites_graphic_overall_container">{shouldDisplayGripicType}</div>

      {/* <div className="house_utites_graphic_all_container"> */}
      {shouldDisplayGripicType2}
      {/* </div> */}
      <dvi>{shouldDisplayGripicType3}</dvi>
    </div>
  )
}
